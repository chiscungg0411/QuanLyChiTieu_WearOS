...
class TransactionService {
  ...
  Stream<List<TransactionDoc>> get transactionsStream {
    ...
    if (ref == null) {
      debugPrint('[WearOS TransactionService] transactionsStream: user not logged in');
      return Stream.value([]);
    }
    
    debugPrint('[WearOS TransactionService] transactionsStream: subscribing to Firestore...');
    return ref
        .orderBy('thoiGian', descending: true)
        .snapshots()
        .map((snapshot) {
          debugPrint('[WearOS TransactionService] Received ${snapshot.docs.length} transactions');
  ...
  Future<void> add({
    ...
    final ref = _transactionsRef;
    if (ref == null) {
      debugPrint('[WearOS TransactionService] Cannot add: user not logged in');
      return;
    }

    debugPrint('[WearOS TransactionService] Adding: muc=$muc, soTien=$soTien');
    try {
      await ref.add({
        'muc': muc,
        'soTien': soTien,
      ...
      debugPrint('[WearOS TransactionService] Added successfully!');
    } catch (e) {
      debugPrint('[WearOS TransactionService] Error: $e');
    }
  }
